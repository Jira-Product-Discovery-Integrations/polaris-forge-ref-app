...
--data-raw '{"query":"mutation setExternalAuthCredentials($input: SetExternalAuthCredentialsInput!) {\n  setExternalAuthCredentials(input: $input) {\n    success\n    statusCode\n    message\n  }\n}","variables":{"input":{"environment":{"appId":"ari:cloud:ecosystem::app/'"$FORGE_APP_ID"'","key":"'"$FORGE_ENV"'"},"serviceKey":"'"$PROVIDER_SERVICE_KEY"'","credentials":{"clientId":"'"$PROVIDER_CLIENT_ID"'","clientSecret":"'"$PROVIDER_CLIENT_SECRET"'"}}}}'